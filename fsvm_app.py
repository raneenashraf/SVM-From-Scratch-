...
class SoftMarginSVM:
        ...
        try:
            if self.optimization == 'qp':
                ...
                for epoch in range(epochs):
                    ...
                    # Check for NaN values in margins
                    if np.any(np.isnan(margins)):
                        raise ValueError("NaN values encountered in margin calculation.")
    ...
    def plot_confusion_matrix(self, X, y):
        ...
        if np.isnan(y_pred).any():
         raise ValueError("Predictions contain NaN values. Check model training or kernel parameters.")
...
class KernelSVM:
    def __init__(self, kernel='rbf', C=1.0, gamma='scale', degree=3, coef0=0.0, 
        ...
        self._b_history = []
        self._w_history=[]
        self._support_vector_indices = []

    def get_support_vectors(self):
        return self.support_vectors, self._support_vector_indices

    def _initialize_gamma(self, X):
        if isinstance(self.gamma, str):
            if self.gamma == 'scale':
                self._gamma_value = 1 / (X.shape[1] * np.var(X))
            elif self.gamma == 'auto':
                self._gamma_value = 1 / X.shape[1]
            else:
                raise ValueError("Invalid gamma string")
        else:
    ...
    def _validate_data(self, X, y):
        ...
        if set(np.unique(y)) != {-1, 1}:
            raise ValueError("y must be encoded as -1 and 1")
        return X, y
    ...
    def _rbf_kernel(self, X1, X2):
        if self._gamma_value is None:
            self._initialize_gamma(X1)
        K = -2 * X1 @ X2.T
        K += np.sum(X2**2, axis=1)
        K += np.sum(X1**2, axis=1)[:, np.newaxis]
        return np.exp(-self._gamma_value * K)

    def _poly_kernel(self, X1, X2):
     coef0 = self.coef0 if hasattr(self, 'coef0') and self.coef0 is not None else 1
     degree = self.degree if hasattr(self, 'degree') and self.degree is not None else 3

     kernel_matrix = (X1 @ X2.T + coef0) ** degree
    
     if np.isnan(kernel_matrix).any():
        raise ValueError("NaN detected in polynomial kernel output!")
    ...
    def fit(self, X, y):
        ...
        if self.kernel in ['rbf']:
            if self.gamma == 'scale':
                self._gamma_value = 1.0 / X.shape[1]
            elif self.gamma == 'auto':
    ...
    def _fit_qp(self, X, y):
        ...
        self._support_vector_indices = np.where(sv)[0]
    ...
    def _fit_gradient(self, X, y):
        ...
        for it in range(self.max_iter):
            margin = y * (K @ (self.alpha * y) + self.b)
            if self.optimizer == 'sgd':
                loss = np.mean((np.maximum(0, 1 - margin))**2)
                grad_alpha = -2 * (1 - margin) * y * K @ y / n
                grad_b = -2 * np.mean((1 - margin) * y)
            else:
                loss = np.mean(np.maximum(0, 1 - margin))
                grad_alpha = -np.where(margin < 1, y * K @ y, 0)
                grad_b = -np.mean(np.where(margin < 1, y, 0))
    ...
    def plot_accuracy_curve(self, X_test, y_test, eval_every=10):
        if not hasattr(self, "_alpha_history") or not hasattr(self, "_b_history"):
            st.warning("Accuracy history is not available. Please train the model first.")
            return
        
        acc_list = []
        iterations = range(0, len(self._alpha_history), eval_every)

        for i in iterations:
            alpha = self._alpha_history[i]
            b = self._b_history[i]
            y_pred = self._decision_function(X_test, alpha, b)
            acc = accuracy_score(y_test, np.sign(y_pred))
            acc_list.append(acc)

        if acc_list:  # Plot only if acc_list has values
            fig, ax = plt.subplots()
            ax.plot(list(iterations), acc_list)
            ax.set_xlabel("Iterations")
            ...
            st.warning("Accuracy list is empty. Check if training ran correctly.")
    ...
    def _decision_function(self, X, alpha, b):
        K = self._kernel(X, self.X_train)
        sv = alpha > 1e-5
        return K[:, sv] @ (alpha[sv] * self.y_train[sv]) + b

    def _predict_with_params(self, X, alpha, b):
        K = self._kernel(X, self.X_train)
        sv = alpha > 1e-5
        return np.sign(K[:, sv] @ (alpha[sv] * self.y_train[sv]) + b)
    ...
    def compare_methods(self, methods=None, kernel='rbf', C=1.0):
        ...
        for method in methods:
            ...
            for i, acc in enumerate(acc_list):
                      y_pred = self._decision_function(self.X_test, self._w_history[i], self._b_history[i])
...
def plot_feature_pairs(X, y, features):
    ...
    "blue_white_orange", ["blue", "white", "orange"]
)
        ax = axes[i]
        ax.scatter(X[:, f1], X[:, f2], c=y, cmap=custom_cmap, alpha=0.6)
...
def main():
    ...
    st.image(r"C:\Users\ranee\Dropbox\PC\Desktop\SVM\svm-1-.jpg", width=1000)
    ...
    with col2:
     ...
     fig, ax = plt.subplots()
    # Specify colors as a list for the pie chart
     df[target_col].value_counts().plot(kind='pie', autopct='%1.1f%%', ax=ax, colors=['orange', 'blue'])
     st.pyplot(fig)
    ...
    "blue_white_orange", ["blue", "white", "orange"]
        ...
        fig, ax = plt.subplots()
        ax.scatter(df[selected_x], df[selected_y], c=df[target_col], cmap=custom_cmap, alpha=0.6)
    ...
    st.header("🤖 3. SVM Models")
    tab1, tab2, tab3 , tab4 , tab5= st.tabs(["Linear SVMs", "Kernel SVMs", "Model Comparison","Prediction","NOTEBOOK"])
    ...
    with tab2:
        ...
        # Create columns for the kernel models
        col_kernel1, col_kernel2 = st.columns(2)
...
#+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    with tab4:
        ...
        model_sources = {
            'Hard Margin SVM': getattr(st.session_state, 'hard_margin_model', None),
            'Soft Margin SVM': getattr(st.session_state, 'soft_margin_model', None),
            'RBF Kernel SVM': getattr(st.session_state, 'rbf_model', None),
            'Polynomial Kernel SVM': getattr(st.session_state, 'poly_model', None)
        }

        for name, model in model_sources.items():
            if model and name not in st.session_state.trained_models:
                st.session_state.trained_models[name] = model

        st.markdown("### 1. Model Selection & Training")

        model_options = list(model_sources.keys())
        available_models = [m for m in model_options if m in st.session_state.trained_models]

        if not available_models:
            st.warning("Alarm: If you want to make a predictions with any model, uou need to run(Train)it first.")
            available_models = model_options

        model_type = st.selectbox("Select SVM model type:", available_models, index=0)

        st.markdown("#### Model Parameters")

        if model_type == "Hard Margin SVM":
            st.info("""
            **Hard Margin SVM Characteristics:**
            ...
            if model_type == "Soft Margin SVM":
                with param_col1:
                    C = st.slider("Regularization (C)", 0.01, 10.0, 1.0)
                with param_col2:
                    opt_method = st.selectbox(
                        "Optimization Method",
                        ["Quadratic Programming", "Gradient Descent"],
                        key='soft_opt_method')

            elif model_type == "RBF Kernel SVM":
                with param_col1:
                    C = st.slider("Regularization (C)", 0.01, 10.0, 1.0)
                with param_col2:
                    gamma = st.slider("Gamma (γ)", 0.001, 1.0, 0.1)

            elif model_type == "Polynomial Kernel SVM":
                with param_col1:
                    C = st.slider("Regularization (C)", 0.01, 10.0, 1.0)
        ...
        if st.button(f"Train {model_type}" if model_type not in st.session_state.trained_models else "Retrain Model"):
            with st.spinner(f"Training {model_type}..."):
                try:
                    if model_type == "Hard Margin SVM":
                        model = HardMarginSVM()
                    elif model_type == "Soft Margin SVM":
                        opt_map = {"Quadratic Programming": "qp", "Gradient Descent": "gd"}
                        model = SoftMarginSVM(C=C, optimization=opt_map[opt_method])
                    elif model_type == "RBF Kernel SVM":
                        model = KernelSVM(kernel='rbf', C=C, gamma=gamma)
                    elif model_type == "Polynomial Kernel SVM":
                        model = KernelSVM(kernel='poly', C=C, degree=degree, gamma=gamma)
                    ...
                    st.metric("Training Accuracy", f"{train_acc:.5f}")
                    ...
                    error_msg = str(e)
                    if "not linearly separable" in error_msg:
                        st.error("""
                        ❌ Hard Margin SVM requires perfectly separable data!
                        Solutions:
                        1. Use Soft Margin SVM instead
                        2. Check/clean your data
                        3. Standardize features
                        """)
                    else:
                        st.error(f"Training failed: {error_msg}")

        st.markdown("### 2. Make Predictions")
        ...
        if st.session_state.trained_models:
            ...
            if st.button("Predict", type="primary"):
                ...
                try:
                    ...
                    decision_value = model.decision_function(input_scaled)[0] if hasattr(model, 'decision_function') else None

                    st.markdown("## 🎯 Prediction Results")
                    st.markdown(""" What is a Decision Value?
                                   **Definition**: A numerical score output by a classifier indicating the model's confidence in a prediction.
                                    - **Key Properties**:
                                    - **Sign**: Determines the predicted class (e.g., positive/negative).
                                    - **Magnitude**: Reflects confidence (higher absolute value = stronger confidence
                                    ## Common Use Cases
                                    1. **Support Vector Machines (SVM)**:
                                       - Distance from the hyperplane (`|value|` = margin width).
                                        - Example: `2.46157` → Far from the decision boundary (high confidence).
                                    2. **Binary Classification**:
                                    - Positive value → Class "1".
                                    - Negative value → Class "0" (or vice versa, depending on implementation)""")


                    col1, col2 = st.columns(2)
                    with col1:
                        if prediction == 1:
                            st.success(f"Predicted Class: Positive (1)")
                        else:
                            st.error(f"Predicted Class: Negative (-1)")
                        if decision_value is not None:
                            st.write(f"Decision Value: {decision_value:.5f}")

                    if len(features) >= 2:
                        fig = plot_decision_boundary(model, X_train, y_train, features)
                        ax = fig.gca()
                        ax.scatter(
                            input_scaled[0,0], input_scaled[0,1],
                            c='green' if prediction==1 else 'red',
                            marker='*', s=300, edgecolor='black', linewidth=2,
                            label=f'Prediction (Value: {decision_value:.5f})' if decision_value else 'Prediction'
                        )
                        ax.legend()
                        st.pyplot(fig)
        ...
        st.markdown("### 3. Model Management")
        if st.session_state.trained_models:
            st.write("Current trained models:")
            for name in list(st.session_state.trained_models):
                cols = st.columns([4,1])
                cols[0].write(f"- {name}")
                if cols[1].button(f"Delete {name}"):
    ...
    with tab5:
        ...
        st.markdown("### Jupyter Notebook Output")

        try:
           with open(r"C:\Users\ranee\Dropbox\PC\Desktop\SVM\convex_project.html", 'r', encoding='utf-8') as file:
            html_content = file.read()
            components.html(html_content, height=800, scrolling=True)
        except FileNotFoundError:
           st.error("Notebook file not found. Make sure it's named 'notebook.html' and in the same folder.")
           
    st.markdown("### Created by :")
    st.image(r"C:\Users\ranee\Dropbox\PC\Desktop\SVM\RaneenLogo.png", width=400)
